...
class Rifamax::Raffle < ApplicationRecord
  ...
  def validate_payment_before_selling!
    return if admin_status == 'payed'
    return if expired_date.present? && expired_date > Date.current

    raise StandardError.new "You can't sold tickets if you don't pay the raffle first"
  end
  ...
  def pay_triple_body(payload = {}, tokenspj, subdomain, jwt, raffle_id, payment_info)
    ...
    unless @result.code == 200
      raise StandardError, JSON.parse({ res: @result, message: "Something failed in payment of triple", code: @result.code, req_body: payload }.to_json)
    else
      if @result["cmd"] == 'C10'
  ...
  def refund_triple_body(payload = {}, tokenspj, subdomain, jwt, raffle_id)
    ...
    unless @result.code == 200  
      raise StandardError, JSON.parse({ message: "Something failed reversing triple", data: @result.body, code: @result.code, req_body: payload }.to_json)
    else
      if @result["cmd"] == 'C16'
